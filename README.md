...
1. **Object storage.** Any S3-compatible bucket. Cloudflare R2 is the cheap default (10 GB free, no egress fees): create a bucket, then *Manage R2 API Tokens* → token with *Object Read & Write* on that bucket; it shows the access key pair and the endpoint `https://<account-id>.r2.cloudflarestorage.com`. Use `AWS_REGION=auto` for R2. Plain AWS S3 works the same with an IAM key and the endpoint left empty.
2. **Push the lake.** After a local backfill, copy it up once with the AWS CLI (`brew install awscli`, `aws configure` with that key pair): `aws s3 sync ./data s3://BUCKET/filings-hub --exclude "raw/*"`, adding `--endpoint-url https://<account-id>.r2.cloudflarestorage.com` for R2. `raw/` can stay local; nothing downstream needs it once the lake is built.
