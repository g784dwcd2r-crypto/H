...
Every FSDS table load keeps every row and every column of the SEC's file: the columns every vintage carries are typed, the rest pass through as text, and `num` rows broken down by an axis (a business segment, a geography, one investment of a fund) load with `dimensional = true` so the statements builder can take the line totals while everything else stays queryable. A line the SEC wrapped in double quotes because a value held a tab is re-joined before reading (`repaired_rows`). Each load is reconciled and recorded in `fsds/load_log/`: raw rows in the file, rows loaded, rows repaired, rows DuckDB could not read (with the first examples), values present but unparseable, and the encoding used. `filings-hub fsds 2022q4 2024q4` (or `--all`) reloads quarters from the raw zips after a loader change; `filings-hub statements 2026q2` (or `--all`) rebuilds the as-reported statements from the loaded quarters after a builder change, without reading the zips again. Quarters before about 2013 carry Windows-1252 bytes; when UTF-8 rejects a row the loader transcodes the file line by line (valid UTF-8 lines untouched, the rest decoded as Windows-1252) and reads it again, so no row is lost to encoding. A table with more than 0.1 % of its rows rejected is flagged in the backfill's `run_log` failures.
