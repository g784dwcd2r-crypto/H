...
export default function ProjectWorkspace({ initialProject, initialNotes, accountId }: { initialProject: Project; initialNotes: ProjectNote[]; accountId: string }) {
  ...
  return <div className="project-workspace-page">
    ...
    <div className="project-workspace">
      <aside className="project-notes-nav" aria-label="Project notes"><div><h2>Notes & theses</h2><button className="linkbtn" disabled={busy || !draftReady} onClick={() => open(null)}>+ New note</button></div>{notes.length ? <ul>{notes.map(note => <li key={note.id}><button onClick={() => open(note)} disabled={busy || !draftReady} aria-current={current?.id === note.id ? "true" : undefined}><span>{note.kind === "thesis" ? "Thesis" : "Note"}</span><strong>{note.title}</strong><small>{note.citations.length} source {note.citations.length === 1 ? "reference" : "references"}</small></button></li>)}</ul> : <p className="muted">No saved notes yet. Write your first note here.</p>}<Link href="/research">Find source documents ↗</Link></aside>
      <section className="note-editor" aria-label="Research note editor" aria-busy={!draftReady}>
        <div className="note-editor-top"><label>Type<select aria-label="Note type" disabled={!draftReady} value={draft.kind} onChange={e => change({ kind: e.target.value as Draft["kind"] })}><option value="note">Research note</option><option value="thesis">Working thesis</option></select></label><span>{dirty ? "Unsaved changes" : current ? `Saved · revision ${current.revision}` : "New draft"}</span></div>
        ...
        <label>Title<input aria-label="Note title" disabled={!draftReady} value={draft.title} onChange={e => change({ title: e.target.value })} maxLength={200} placeholder="Give your research a title"/></label>
        <label>Your analysis<textarea aria-label="Your analysis" disabled={!draftReady} value={draft.body} onChange={e => change({ body: e.target.value })} maxLength={100000} rows={14} placeholder={draft.kind === "thesis" ? "Your thesis, supporting evidence, counterarguments and what would change your mind…" : "Write your observations, questions and next steps…"}/></label>
        ...
        <div className="note-save-actions"><button className="btn" type="button" disabled={!draftReady || busy || !draft.title.trim() || conflict || !!current && !dirty} onClick={() => void save()}>{busy ? "Saving…" : current ? "Save changes" : "Save note"}</button>{current && project.can_manage && <button type="button" className="linkbtn danger-action" disabled={busy} onClick={async () => { if (!window.confirm(`Delete “${current.title}”? This cannot be undone.`)) return; setBusy(true); setError(null); try { const response = await projectRequest<{ project_revision: number }>(`${base}/notes/${current.id}?expected_revision=${current.revision}`, "DELETE"); setNotes(list => list.filter(note => note.id !== current.id)); updateProjectRevision(response.project_revision); setCurrent(null); setDraft(empty()); setDirty(false); setNotice("Note deleted."); } catch (e) { setError(e instanceof Error ? e.message : "The note could not be deleted."); } finally { setBusy(false); } }}>Delete note</button>}<span>{current?.authorship === "template" ? "Template prompts · add your own analysis" : "Your writing · no generated analysis"}</span></div>
      </section>
    </div>
  </div>;
}
