...
type View = "demo-requests" | "launch-memberships" | "overview" | "users" | "organizations" | "research" | "jobs" | "sources" | "configuration" | "audit" | "security";
type Review = { path: string; action: string; target: string; label: string; summary: string; values: Data };
const views: [View, string][] = [["overview", "Overview"], ["demo-requests", "Enquiries"], ["launch-memberships", "Founding group"], ["users", "Accounts"], ["organizations", "Organizations"], ["research", "Research index"], ["jobs", "Research jobs"], ["sources", "Source readiness"], ["configuration", "Configuration"], ["audit", "Audit history"], ["security", "Operator security"]];
...
export default function AdminConsole() {
  ...
  async function load(selected = view, query = search, start = offset) {
    ...
    setLoading(true);
    try {
      const result = await call(`${selected}?q=${encodeURIComponent(query)}&limit=25&offset=${start}${["demo-requests", "launch-memberships"].includes(selected) ? "&status=" + encodeURIComponent(leadStatus) : ""}`);
  ...
  useEffect(() => { if (session && !session.must_change_password) void load(); }, [session?.username, session?.must_change_password, view, search, offset, leadStatus]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  function navigate(next: View) {
    ++sequence.current; setView(next); setData(null); setDetail(null); setSearch(""); setQ(""); setOffset(0); setLeadStatus(""); setError(""); setNotice("");
  }
  ...
  return <div className={styles.console}>
    ...
    {initializing ? <div className={styles.auth}><p role="status">Checking administrator session…</p></div> : loginOrChange ? <section className={styles.auth}>
      ...
      <section className={styles.content}>
        ...
        {["demo-requests", "launch-memberships", "users", "organizations", "research", "jobs", "audit"].includes(view) && <form className={styles.search} onSubmit={event => { event.preventDefault(); setSearch(q); setOffset(0); if (q === search && offset === 0) void load(); }}><label className={styles.srOnly} htmlFor="admin-search">Search {labels[view]}</label><input id="admin-search" value={q} maxLength={150} placeholder={`Search ${labels[view].toLowerCase()}…`} onChange={event => setQ(event.target.value)} /><button>Search</button></form>}
        {["demo-requests", "launch-memberships"].includes(view) && <div className={styles.leadFilter}><label>Filter by status<select value={leadStatus} disabled={busy} onChange={event => { setLeadStatus(event.target.value); setOffset(0); }}><option value="">All statuses</option>{(view === "demo-requests" ? ["new", "contacted", "scheduled", "completed"] : ["reserved", "waitlisted"]).map(status => <option key={status} value={status}>{label(status)}</option>)}</select></label><p className={styles.muted}>{view === "demo-requests" ? "Demo and contact requests. Status changes track handling; they do not send an email or book a meeting." : "Verified accounts that explicitly joined. All regions share one allocation of twenty individual places."}</p></div>}
        {view === "demo-requests" && data && <><div className={styles.tableScroll}><table><thead><tr><th>Person / organisation</th><th>Enquiry</th><th>Region / received</th><th>Status</th></tr></thead><tbody>{tableItems.map(row => <tr key={row.id}><td><strong>{row.name}</strong><small>{row.email}</small><small>{row.organisation || "Independent / not provided"}</small></td><td><details><summary>Read enquiry</summary><p className={styles.enquiryText}>{row.workflow}</p><small>Reference {row.id}</small></details></td><td>{label(row.region)}<small>{date(row.created_at)}</small></td><td><Badge value={row.status}/><select aria-label={`Change enquiry status for ${row.email}`} value="" disabled={busy || loading} onChange={event => { const status = event.target.value; if(status) prepare({path:`demo-requests/${row.id}/status`,action:"demo.status",target:row.id,label:"Update enquiry status",summary:`${row.name} (${row.email}): ${label(row.status)} → ${label(status)}. This records your handling status only.`,values:{status,expected_revision:row.revision}}); }}><option value="">Update status…</option>{["new","contacted","scheduled","completed"].filter(status=>status!==row.status).map(status=><option key={status} value={status}>{label(status)}</option>)}</select></td></tr>)}</tbody></table></div>{tableItems.length===0&&<Empty>No matching enquiries. New demo and contact requests appear here after successful submission.</Empty>}</>}
        {view === "launch-memberships" && data && <><div className={styles.metrics}><article><span>Reserved individual places</span><strong>{data.campaign?.allocated ?? "—"} / {data.campaign?.capacity ?? "—"}</strong></article><article><span>Places remaining</span><strong>{data.campaign?.remaining ?? "—"}</strong></article><article><span>Reservation period</span><Badge value={data.campaign?.state}/><small>Closes {date(data.campaign?.registration_closes_at)}</small></article></div><div className={styles.tableScroll}><table><thead><tr><th>Account</th><th>Status</th><th>Region</th><th>Joined</th><th>Free access</th></tr></thead><tbody>{tableItems.map(row=><tr key={row.id}><td>{row.email || row.user_id}<small>{row.name || ""}</small></td><td><Badge value={row.status}/></td><td>{row.region}</td><td>{date(row.joined_at)}</td><td>{row.status==="reserved"?<>{date(row.benefit_starts_at)}<small>Until {date(row.benefit_ends_at)} (exclusive)</small></>:"Waiting list · no entitlement"}</td></tr>)}</tbody></table></div>{tableItems.length===0&&<Empty>No matching founding memberships.</Empty>}</>}
        {view === "overview" && data && <>
