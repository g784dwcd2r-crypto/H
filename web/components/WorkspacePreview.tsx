    ...
    <div className="preview-frame" id="platform">
      <div className="preview-workspace">
        <aside className="preview-sidebar" aria-label="Example workspace shortcuts">
          ...
          <span className="preview-caption">Select a number.<br />Follow its source.</span>
        </aside>
        <div className="preview-main">
          ...
          {tab === "Financials" ? <div key="financials" role="tabpanel" aria-label="Example financials">
            ...
            <div className="preview-evidence" aria-live="polite"><DocumentIcon /><div key={`${selected.row}-${selected.year}`} className="preview-observation"><strong>{metric.label} · FY {2024 - selected.year}</strong><span>Apple FY2024 results · Financial statements, p. 1</span></div><a href={EXAMPLE_SOURCE} target="_blank" rel="noreferrer">View source ↗</a></div>
          </div> : tab === "Filings" ? <div key="filings" className="preview-documents" role="tabpanel" aria-label="Example filings"><p className="eyebrow">Fiscal year 2024</p><h3>One period. Related filings.</h3><a href={EXAMPLE_ANNUAL} target="_blank" rel="noreferrer"><DocumentIcon /><span>Annual report<small>Form 10-K · November 1, 2024</small></span><span>↗</span></a><a href={EXAMPLE_RELEASE} target="_blank" rel="noreferrer"><DocumentIcon /><span>Earnings release<small>Fourth quarter results · October 31, 2024</small></span><span>↗</span></a></div> : <div key="overview" className="preview-overview" role="tabpanel" aria-label="Example overview"><p className="eyebrow">Fiscal year 2024 · Historical example</p><h3>A clear starting point.</h3><p>Explore the reported numbers, then follow the evidence into the original document.</p><div><span>Total net sales<strong>$391,035m</strong></span><span>Net income<strong>$93,736m</strong></span></div><button className="text-link" onClick={() => setTab("Financials")}>Inspect the financials <ArrowIcon /></button></div>}
        </div>
      </div>
    </div>
  );
