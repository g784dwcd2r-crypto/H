...
import { safeSourceUrl, type ResearchResult } from "@/lib/research";

export default function ResearchResults({ results }: { results: ResearchResult[] }) {
  ...
  return <div className={"research-results-layout" + (selected ? " with-evidence" : "")}>
    ...
    {selected && <aside id="research-evidence" className="research-evidence" aria-label="Selected search result evidence">
      ...
      <Link className="btn secondary" href={`/research/documents/${encodeURIComponent(selected.version_id)}`}>Read this indexed version</Link>
