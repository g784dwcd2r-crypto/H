...
export async function POST(req: NextRequest) {
  ...
  if (!body || typeof body !== "object" || Array.isArray(body)) return Response.json({ detail: "Provide an email address." }, { status: 400 });
  const next = safeAuthNext(body.next);
  const region = authRegion(body.region, req.cookies.get(REGION_COOKIE)?.value);
  try {
    const r = await api.post("/auth/magic-link", { email: body.email, ...(next !== "/" ? { next } : {}), ...(region ? { region } : {}) });
    ...
    return response;
  } catch { return Response.json({ detail: "Email sign-in is temporarily unavailable. Please try again." }, { status: 503 }); }
}
