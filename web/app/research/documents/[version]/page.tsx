...
import { documentVersionParam, indexedPageTexts, safeSourceUrl, type IndexedDocument } from "@/lib/research";
...
export default async function IndexedDocumentPage({ params, searchParams }: { params: Promise<{ version: string }>; searchParams: Promise<{ before?: string; history_offset?: string }> }) {
  ...
  return <article className="indexed-document">
    <p className="crumb"><Link href={`/research?cik=${doc.cik}`}>← Search {doc.company_name || `CIK ${doc.cik}`} filings</Link></p>
    ...
    <DocumentHistory version={normalizedVersion} before={query.before} offset={/^\d{1,6}$/.test(query.history_offset || "") ? Number(query.history_offset) : 0}/>
