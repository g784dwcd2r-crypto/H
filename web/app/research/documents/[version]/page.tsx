...
export default async function IndexedDocumentPage({ params, searchParams }: { params: Promise<{ version: string }>; searchParams: Promise<{ before?: string; history_offset?: string; projection?: string; return_to?: string }> }) {
  const { version } = await params;
  const query = await searchParams;
  const normalizedVersion = documentVersionParam(version);
  if (!normalizedVersion) notFound();
  const projection = query.projection && /^projection:[0-9a-f]{64}$/.test(query.projection) ? query.projection : null;
  const response = await workspaceRequest<IndexedDocument>(projection ? `/research/projections/${encodeURIComponent(projection)}` : `/research/documents/${encodeURIComponent(normalizedVersion)}`);
  ...
  if (doc.version_id !== normalizedVersion) notFound();
  ...
  return <article className="indexed-document">
    ...
    <div className="indexed-document-actions">{original && <a className="btn secondary" href={original} target="_blank" rel="noreferrer">Open original document ↗</a>}<Link className="btn secondary" href={`/research/ask?version=${encodeURIComponent(doc.version_id)}`}>Ask about this document</Link><SaveSource documentId={doc.document_id} versionId={doc.version_id} title={doc.title || doc.filename}/><span>Text captured {doc.indexed_at}</span></div>
    ...
    <div className="notice">{projection ? "This is an immutable text projection of the captured source bytes. Older projections remain inspectable." : "This is the extracted text used for search, preserved as an indexed version."} Tables and layout may differ from the original. Use the original document to verify formatting and figures.</div>
